  ...
  return (
    <div className="h-full flex flex-col bg-[#0A0A0A] font-mono overflow-hidden">
            ...
            <table className="w-full text-[10px] border-collapse">
              <thead className="sticky top-0 bg-[#0A0A0A] z-10">
                <tr className="border-b border-neutral-800">
                  <th className="text-left px-3 py-2 text-neutral-600 font-normal w-10">#</th>
                  <th className="text-left px-3 py-2 text-neutral-600 font-normal">TICKER</th>
                  <th className="text-right px-3 py-2 text-neutral-600 font-normal">QTD</th>
                  <th className="text-right px-3 py-2 text-neutral-600 font-normal">CUSTO</th>
                  <th className="text-right px-3 py-2 text-neutral-600 font-normal">ACTUAL</th>
                  {[
                    ['valorActual', 'VALOR'],
                    ['pnlTotal',    'P&L $'],
                    ['pnlPct',      'P&L %'],
                  ...
                  <th className="text-right px-3 py-2 text-neutral-600 font-normal">PESO</th>
                  <th className="text-right px-3 py-2 text-neutral-600 font-normal">SECTOR</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-1.5">
                        ...
                        <div className="text-[9px] text-neutral-600 truncate max-w-[120px]">{p.nome}</div>
                      </td>
                      <td className="px-3 py-1.5 text-right text-neutral-400">{p.quantidade.toLocaleString('en-US')}</td>
                      <td className="px-3 py-1.5 text-right text-neutral-500">${p.custoMedio.toFixed(p.custoMedio < 100 ? 4 : 2)}</td>
                      ...
                      <td className="px-3 py-1.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <span className="text-neutral-500">{peso.toFixed(1)}%</span>
                        </div>
                      </td>
        ...
        {abaDados === 'sector' && (
          <div className="flex-1 flex min-h-0">
            ...
            <div className="w-64 shrink-0 flex flex-col items-center justify-center p-4">
              <div className="text-[9px] text-neutral-600 mb-2">ALOCAÇÃO SECTORIAL</div>
            ...
            <div className="flex-1 overflow-auto p-4">
              {dadosSector.map((s) => (
                <div key={s.name} className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-[10px] text-neutral-500 w-10 text-right">{s.pct.toFixed(1)}%</span>
          ...
          <div className="flex-1 p-6 overflow-auto">
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="border border-neutral-800 rounded p-4">
                <div className="text-[9px] text-neutral-600 mb-1">BETA PORTFOLIO</div>
              ...
              {/* Diversificação */}
              <div className="border border-neutral-800 rounded p-4">
                <div className="text-[9px] text-neutral-600 mb-1">DIVERSIFICAÇÃO</div>
                <div className="text-2xl font-black text-white">{dadosSector.length}</div>
                <div className="text-[10px] text-neutral-600 mt-1">sectores distintos</div>
              </div>

              {/* Posição maior */}
              <div className="border border-neutral-800 rounded p-4">
                <div className="text-[9px] text-neutral-600 mb-1">MAIOR POSIÇÃO</div>
                {(() => {
                  ...
                  {dadosComputados.sort((a, b) => Math.abs(b.beta) - Math.abs(a.beta)).map((p) => {
                    ...
                    return (
                      <div key={p.ticker} className="flex items-center gap-3 text-[10px]">
                        ...
                        <span className="text-neutral-600">Contribuição: {(exposicao * 100).toFixed(2)}%</span>
                      </div>
                    )
                  })}
                </div>
      ...
      <div className="flex items-center gap-4 px-4 py-1.5 border-t border-neutral-800 shrink-0 text-[10px] text-neutral-600">
        ...
        <TrendingUp size={10} className="text-neutral-700" />
        <TrendingDown size={10} className="text-neutral-700" />
      </div>
    </div>
  )
}
